...
export default function SiteFooter() {
  const bitfanLabel = <span className="bitfan-button-copy"><span className="bitfan-button-eyebrow">Lumière</span><span className="bitfan-button-name">Bitfan</span></span>;
    ...
    <>
      ...
      <div className="footer-brand"><span className="brand-logo"><img src="/assets/logo-header.jpg" alt="WLD" /></span><div><strong>一般社団法人ヲタ芸普及協会</strong><small>OFFICIAL PERFORMANCE TEAM / LUMIÈRE</small></div></div>
        ...
        <div className="footer-connect">
          ...
          {bitfanUrl ? (
            <a className="bitfan-button" href={bitfanUrl} target="_blank" rel="noopener noreferrer" aria-label="LumièreのBitfanを開く（新しいタブ）">
              {bitfanLabel}<span className="bitfan-button-arrow" aria-hidden="true">↗</span>
            </a>
          ) : (
            <button className="bitfan-button" type="button" disabled>
              {bitfanLabel}<span className="bitfan-button-status">準備中</span>
            </button>
          )}
        </div>
